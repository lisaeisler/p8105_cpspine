...
```
## Graph of pre-operative factors and homedischarge (see if we need it at team meeting)

 NEED TO DISCUSS WITH THE GROUP
## Heatplot for different co-morbidity and association with the outcome
...
```
* Cell with only 2 data points we may need to take out from the model (unstable model.
```{r, message=FALSE, warning=FALSE}
xtabs(~ home_discharge1+ ventilat1, data = model_data)
```
* cell with only 5 data points may consider to take out (unstable model)
```{r, message=FALSE, warning=FALSE}
...
## We want to see how well our model fits. The output produced by summary(mylogit) has parameters that can be used to check the model fit.The null and deviance residuals and the AIC. One measure of model fit is the significance of the overall model. This test asks whether the model with predictors fits significantly better than a model with just an intercept (i.e., a null model). 
Finally, the p-value can be obtained using:

```{r, message=FALSE, warning=FALSE}
with(mylogit, pchisq(null.deviance - deviance, df.null - df.residual, lower.tail = FALSE))
```
The chi-square has a p-value of less than 0.001 tells us that our model as a whole fits significantly better than an empty model. 
